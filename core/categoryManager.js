...
export class CategoryManager {
    ...
    buildCategoryTabs(categories, extensionDir) {
        ...
        for (const category of categories) {
            ...
            button.connect('clicked', () => {
                console.log(`emoji-picker: Button clicked for category: "${category}"`);
                this.setCategory(category);
            });

            this.#categoryButtons.set(category, button);
            tabBox.add_child(button);
            console.log(`emoji-picker: Added button for "${category}", tabBox now has ${tabBox.get_n_children()} children`);
        }

        this.updateCategoryStates();
        console.log(`emoji-picker: Final tabBox children count: ${tabBox.get_n_children()}`);
        return tabBox;
    }
    ...
    setCategory(category) {
        console.log(`emoji-picker: setCategory called with: "${category}"`);
        console.log(`emoji-picker: Previous currentCategory: "${this.#currentCategory}"`);
        this.#currentCategory = category;
        console.log(`emoji-picker: New currentCategory: "${this.#currentCategory}"`);
        this.updateCategoryStates();
    ...
    updateCategoryStates() {
        console.log(`emoji-picker: updateCategoryStates - highlighting category: "${this.#currentCategory}"`);
        console.log(`emoji-picker: Available category buttons: ${Array.from(this.#categoryButtons.keys()).join(', ')}`);
        for (const [category, button] of this.#categoryButtons) {
            if (category === this.#currentCategory) {
                console.log(`emoji-picker: Adding 'active' class to: "${category}"`);
